...
def main():
    ...
    with st.form(key="pilotage_form"):
        ...
        if signal23==2:    
              ...
              commentaires = st.text_area("Commentaires / Remarques (optionnel)", height=100)
             # ================== Upload fichier PDF / image ==================
              st.sidebar.title("Téléversement fichier (optionnel)")
              fichier_upload = st.sidebar.file_uploader(
               "Téléverser un fichier (PDF, TIF, PNG, JPG…)", 
                  type=["pdf", "tif", "tiff", "png", "jpg", "jpeg"]
                  )

            # Initialisation du lien de fichier
              file_link = None

              if fichier_upload is not None:
               # Indiquer le fichier sélectionné
                 st.sidebar.write(f"Fichier sélectionné : {fichier_upload.name}")

               # Vérifier que les credentials existent dans st.secrets
                 if "gcp_service_account" not in st.secrets:
                    st.sidebar.error("Credentials Google absents dans st.secrets['gcp_service_account']. Impossible d'uploader.")
                 else:
                     try:
                         # Récupération et préparation des credentials (une seule fois ici)
                         creds_dict = st.secrets["gcp_service_account"]
                         scopes = ["https://www.googleapis.com/auth/drive", "https://www.googleapis.com/auth/spreadsheets"]
                         credentials = Credentials.from_service_account_info(creds_dict, scopes=scopes)
                         drive_service = build('drive', 'v3', credentials=credentials)

                         # ID du dossier Drive où tu veux stocker les fichiers
                         drive_folder_id = "1HX2-WbFN6jGpEqJxfc-tKQwIp3NyA8RF"

                         # Préparer metadata et media - gérer mimetype absent
                         file_metadata = {'name': fichier_upload.name, 'parents': [drive_folder_id]}
                         mimetype = fichier_upload.type if getattr(fichier_upload, "type", None) else "application/octet-stream"

                         # Récupérer le buffer du fichier et créer l'objet MediaIoBaseUpload
                         buffer = io.BytesIO(fichier_upload.getbuffer())

                         # Montrer un indicateur pendant l'upload
                         with st.spinner("Upload en cours vers Google Drive..."):
                         ...
                         st.sidebar.success(f"Fichier uploadé sur Drive : [Voir le fichier]({file_link})")

                     except Exception as e:
                     # Message d'erreur lisible pour l'utilisateur
                      st.sidebar.error(f"Erreur lors de l'upload sur Drive : {str(e)}")
              ...
              commentaires= nettoyer_chaine(commentaires)
              data={
                  'Honorateur':time,
